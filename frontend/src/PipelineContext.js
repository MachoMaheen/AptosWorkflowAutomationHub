...
export const PipelineProvider = ({ children }) => {
  ...
  const isValidConnection = useCallback((connection) => {
    ...
    console.log('Validating connection:', { source, target, sourceHandle, targetHandle });
    ...
    const getHandleType = (handleId) => {
      ...
      if (lowerHandle.includes('data') || lowerHandle.includes('event-data')) return 'data';
      ...
      if (lowerHandle.includes('address') || lowerHandle.includes('balance')) return 'data';
    ...
    console.log('Connection validation result:', { isPatternValid, validTargets });

    if (!isPatternValid) {
      console.log(`❌ Invalid connection pattern: ${sourceType} → ${targetType}`);
      return false;
    }
  ...
  // Execute workflow
  const executeWorkflow = useCallback(async (nodes, edges, startNodeId) => {
